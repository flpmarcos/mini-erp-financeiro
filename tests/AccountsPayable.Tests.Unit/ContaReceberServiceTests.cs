...
{
    private static (ContaReceberService svc, ContasAPagar.Web.Data.AppDbContext db) Build()
    {
        var db = TestSupport.NewDb();
        return (new ContaReceberService(db, new Mock<IAuditoriaService>().Object), db);
    }

    private static async Task<int> SeedClienteAsync(ContasAPagar.Web.Data.AppDbContext db)
    {
